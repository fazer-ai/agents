...
describe.skipIf(!dbUp)("runAgentTurn", () => {
  ...
  describe("guardrails 'generated' action", () => {
    ...
    // The guardrails call pins `temperature: 0`, and Anthropic's current generation answers 400 to
    // any temperature at all. Analysis is FAIL-OPEN, so that 400 does not surface as a broken
    // guardrail: it surfaces as a guardrail that approves everything. The parameter must therefore
    // not travel to that provider, while the providers that accept it keep the pin.
    test("the guardrails model drops temperature on anthropic and keeps the pin elsewhere", async () => {
      const captured: ResolvedModelConfig[] = [];
      const clean = JSON.stringify({
        violated: false,
        categories: [],
        rationale: "",
      ...
      const capture = (cfg: ResolvedModelConfig): BaseChatModel => {
        if (cfg.model !== GUARD_MODEL) {
          return new FakeListChatModel({ responses: [REPLY] });
        }
        captured.push(cfg);
        return {
          invoke: async () => ({ content: clean }),
        } as unknown as BaseChatModel;
      };
      for (const [provider, convId] of [
        ["anthropic", 966],
        ["openai", 967],
      ] as const) {
        await setGuardrails({
          ...
          input: {
            enabled: true,
            action: "template",
            checks: { toxicity: true, unsafeContent: false },
            templateMessage: "TEMPLATE-IN",
          },
          output: { enabled: false },
        });
        ...
        await runAgentTurn({
          ...
          deps: {
            makeModel: capture,
            makeClient: guardStub([], []),
            checkpointer: new MemorySaver(),
          },
        });
      }
      expect(captured.map((c) => [c.provider, c.temperature])).toEqual([
        ["anthropic", undefined],
        ["openai", 0],
      ]);
    });
