...
- **Com `--apply` grava no banco de produção.** É mutação: mostre a simulação ao usuário e só aplique com o OK dele para aquela execução. Só as linhas cujo valor muda são reescritas, e ganham o carimbo da tabela atual.
- **`--provider` é obrigatório** e tem que ser o provedor que de fato serviu aquele modelo (o mesmo do agente: `openai`, `anthropic`, `google`, `deepseek`, `openrouter`). A linha de uso não guarda o provedor, e o mesmo nome de modelo dá preços diferentes em provedores diferentes. Na dúvida, confira na configuração do agente antes de aplicar.
...
- Uma linha que a tabela atual não sabe precificar fica como está e aparece na contagem. O comando nunca troca um valor por vazio nem um vazio por zero.
