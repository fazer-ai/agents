...
-- (tenant_id, thread_id) CONCURRENTLY (issue #839). Same file as
-- `20260921120000_assert_conversation_indexes_valid`, pointed at `llm_usage`; that file's header
...
STEP 4, ALWAYS: prisma migrate resolve --rolled-back 20260924150100_assert_llm_usage_indexes_valid, then re-deploy. This migration''s own row is FAILED from the moment it raised, so without step 4 the next deploy stops with P3009 no matter what you did about the index.',
      dead;
  END IF;
END $$;
