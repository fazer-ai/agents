...
- **Availability spans SEVERAL calendars** when the instance allows several and the model omits `calendarId` (issue #100): one `freeBusy` request carries every allowed calendar, each is computed on its OWN busy list, and the merged list is chronological with each slot tagged `calendarId`/`calendarLabel`. Calendars are independent providers here, so their busy windows are never pooled: pooling would answer when they are ALL free at once, which is a meeting room, not a clinic. A calendar `freeBusy` reports an error for is dropped and named in `unavailableCalendars` rather than carried as free (an empty busy list means "free all day", and offering a professional whose bookings we cannot see is a double booking); if none could be read the tool refuses, because an empty slot list reads as "fully booked". Booking, rescheduling and cancelling still act on ONE explicit calendar. The per-calendar slot bound applies only when several are searched, so a single-calendar instance returns exactly what it always did.
