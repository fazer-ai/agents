...
> **A DATA migration over a tenant-scoped table needs `SELECT set_config('role', public.fazerai_fleet_role(), true);` before the statement.** Those tables carry `FORCE ROW LEVEL SECURITY`, which subjects even the table OWNER to the tenant policy, and `MIGRATION_DATABASE_URL` is only documented as "superuser **or** owner". On a self-hosted Postgres the migration role is usually a real superuser and the difference never shows; on managed Postgres (RDS/Neon/Supabase) the admin role is typically the owner WITHOUT `rolsuper`, and there a `UPDATE`/`DELETE` across tenants matches **zero rows and reports success**. `set_config(…, true)` rather than `SET ROLE`, because the role's name is resolved by the database and `SET ROLE` takes a literal; the `true` makes it transaction-local, which is what `migrate deploy` gives it (measured: a migration that raises rolls back entirely). Running the same statements by hand in psql needs an explicit `BEGIN`/`COMMIT` around them — in autocommit the role resets when that `SELECT` returns. Schema DDL is unaffected.
>
