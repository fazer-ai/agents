...
**Every mutable column is in one half of the projection or the other.** The audited services split a row in two: what is PROJECTED is identity, policy and shape, safe to keep in a row that is append-only and outlives the record; what is DIGESTED is everything else, folded into one fingerprint by `digestForAudit` (`src/modules/audit/projection.ts`). The split is what makes the trail honest, and being in NEITHER half is the failure mode — a column changes, the projection does not, `projectionMoved` sees nothing, and the edit writes no row at all. Review found five such columns at once on #399's first pass (a tool's headers and schemas, an integration's config VALUES under unchanged keys, an experiment's variant prompts), and each was the ordinary edit its family gets. A column shown through a TRANSFORM is not covered by that alone: a URL projected as its origin, a stdio command as its launcher, a field as `{name, type}` all report less than the column holds, so the whole value goes in the digest as well. `tests/modules/audit-config-families.test.ts` reads the columns out of `prisma/schema.prisma` and fails while one is in neither half, so the next column added to those models has to be placed rather than forgotten. #399.
