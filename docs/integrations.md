...
A catalog entry is data, one of four `kind`s:
...
- `WEBHOOK` — inbound only, no tools of its own: `GENERIC`, the operator's own system speaking back into a conversation (below). It is not offered as a grantable integration in the agent's tool selection, because there is nothing to grant.
...
6. **Dispatch** (`processInboundDelivery`) under a status CAS (`PENDING→PROCESSING→PROCESSED`, re-entry is a no-op). `conversion` → correlate `externalId`→thread via `IntegrationExternalRef` then record `ConversionEvent` idempotently (`ON CONFLICT DO NOTHING`, so the surrounding tx stays alive); an uncorrelated/duplicate conversion is dropped with a log, never invented. `agent_nudge` correlates inside the tx and runs the agent turn after it commits (see "`agent_nudge`" below). `status_update` is a declared seam, no-op for now.
...
- **Registry**, keyed by `catalogType`. Adding an integration = a catalog entry + a mapper + a zod schema + a `registerMapper` line; the core (route-token, auth, ack/async, idempotency, correlation, dispatch) is untouched. `GENERIC` takes our own normalized shape (below); Asaas maps its payment events.

## Known limitations

- **Stranded deliveries are re-dispatched by a sweep** (`src/modules/webhooks/inbound/sweep.ts`, issue #817). Dispatch is detached after the ack, so a process that dies before it runs, or a `processInboundDelivery` that throws (the scoped tx rolls back to `PENDING`), used to leave the row with nothing retrying it, since the sender holds a 2xx. Now a per-tenant `INBOUND_SWEEP` job (armed at boot for every tenant with an inbound instance, and when a tenant creates its first one) runs every two minutes, finds `PENDING` rows received more than five minutes ago and `PROCESSING` rows whose claim went stale by the processor's own rule (`staleClaim`, shared rather than restated), and arms one `INBOUND_REDISPATCH` per row. That job calls `processInboundDelivery`, the same function the route calls, so the claim is still the one compare-and-set (a re-dispatch racing the route or a redelivery takes the row at most once), the five-attempt cap still applies, and a row that exhausted it still ends `FAILED` with the processor's own `attempts-exhausted` dead-letter line. The re-dispatch is its own kind because it can run the agent's nudge turn and the sweep only queries. Its key is the delivery **and** its attempt count, armed `once`: a row that strands again after a re-dispatch claimed it has a new count and gets a new job, while a dispatch that keeps throwing before anything commits (count unchanged) is not re-armed every pass; it runs out its own job's retries (one-minute base) and the scheduler announces the death at `error`. A partial index over the unfinished rows (`inbound_deliveries_unfinished_idx`) keeps each pass off the delivery history. What this does not cover: a nudge that ran and failed is still a finished delivery (the row is `PROCESSED` whatever the nudge did), so nothing re-runs it.
...
After the deterministic core (resolve → PK correlation → status CAS), a domain event of kind `agent_nudge` injects a **normalized, sanitized** system event into the conversation's LangGraph thread through `runAgentNudge` (`src/graph/nudge.ts`), outside the processing transaction. The guardrails are the follow-up's: the ownership gate (a person holding the conversation ⇒ a private note, never a customer message), idempotency on the delivery CAS, the external bytes reaching the model only inside the `⟦external-data⟧` fence, and proactive output through the follow-up and business-hours policy. Two sources wire it today: Asaas (a confirmed payment, when the instance's `notifyOnPayment` is on) and `GENERIC`.

## Generic webhook: the operator's own system speaking back (`GENERIC`, issue #818)

The case: the agent schedules something in the operator's system during a conversation (a report every 30 minutes, a callback when an order ships), and later that system has something to say to the same customer. Posting to Chatwoot directly would need a bot token outside the agent and would bypass everything the agent enforces; this path goes through the agent.

1. **The reference.** An HTTP tool that renders `{{conversation_ref}}` in its URL, headers, query, body or a fixed field hands the operator's system an opaque handle to the current conversation: `cr_` + 24 random bytes, base64url. It is an `IntegrationExternalRef` of kind `conversation_ref`, **stable per (GENERIC instance, conversation)**, minted on the call (`ensureConversationRef`, `src/modules/integrations/conversation-ref.ts`) BEFORE the request leaves and before the tool's ack, so the receiver can never get a ref that does not exist yet, and a refusal sends neither. The tool names the instance the ref is for in `ToolDefinition.conversationRefIntegrationId` (a GENERIC instance of the same tenant; required whenever a template renders the variable, refused otherwise, checked on the effective pair on create and update, and by the MCP dry run). Where there is no conversation (the playground, the editor's test run) or the client is muted, the tool refuses and sends nothing. The variable is HTTP-tool only (`HTTP_TOOL_ONLY_VAR_NAMES`), not a context variable: code tools do not see it.
2. **The event.** The system POSTs to the instance's webhook `{event_id, conversation_ref, text, status?}`: `event_id` is the idempotency key, `text` is at most 4000 characters with line breaks kept. Inbound auth is mandatory (`requiresInboundAuth`: `NONE` is refused on create and update; default `HMAC_SHA256` over the raw body in `x-webhook-signature`), because an open route would let anyone write to every customer that ever had a ref minted.
3. **Correlation is synchronous.** The receptor resolves the ref before persisting: a ref that is unknown, minted by another instance, or whose instance was deleted answers `outcome: "uncorrelated"` and records nothing, so the sender learns to stop instead of filling a table. A ref only correlates on the instance that minted it, and Asaas/toolpack correlation ignores refs of this kind (`toolpackRefThread`).
4. **The turn.** `buildNudge` frames the event as `operator_event`: the directive is to pass the text on to the customer faithfully, the text goes inside the fence with each line quoted as `| ` (so no external line starts at column 0), and the instance's `config.instructions` (at most 2000 characters, edited in the integration screen) ride along as operator guidance. `deliverToResolved`: a conversation the bot resolved still counts as the bot's for this event and is written to without reopening. Who resolved it is read from the recorded origin (`Conversation.resolvedBy`: the agent's `resolve_conversation`, the follow-up's abandonment step or the redirect ladder), never from the assignee, because an operator resolving in Chatwoot leaves the AgentBot assigned and records nothing on our side; that close, and one from our console, is a person's. One held by a person, closed by a person, or `open`, gets the text as it arrived in a private note, written without a model call: the note directive lets a model stay silent, and a report dropped that way leaves nothing anywhere. The same note is written wherever a takeover is found later in the run (during the contact-authorization call, during the wait for the thread, by the tool fence, or by the probes after the model call and after the output guardrail), and the model's undelivered words are taken back out of the thread.

Export and import carry it: a bundle names the tool's GENERIC instance by name, exports that instance, and rewires the tool on import (a missing instance is a warning, and the tool refuses to run until one is picked). Known gaps, tracked separately: a nudge's message is not yet recorded as our side speaking (#816), and a delivery that finished with its nudge refused is not re-run (the stranded ones are, see Known limitations).
