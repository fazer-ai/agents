...
**O REINDEX tem uma precondição: o índice precisa ser CONSTRUÍVEL.** Para índice não-único, que é o caso dos três builds concorrentes desta árvore, isso vale sempre. Índice ÚNICO cujos dados violam a unicidade é a exceção, e ali o REINDEX falha do mesmo jeito que o build falhou **e deixa um segundo índice inválido** (medido: `u_v_idx` mais `u_v_idx_ccnew`, os dois `indisvalid = false`, depois de um `REINDEX INDEX CONCURRENTLY` no índice deixado por uma falha de chave duplicada). Esse caso pede resolver os dados primeiro, ou dropar o índice de propósito. A mensagem da guarda tem que dizer isso, senão manda o operador num comando que piora o catálogo. **E o REINDEX tem espera própria**, por snapshots antigos, e espera por qualquer transação mais velha que ele, inclusive uma que nunca toque a tabela: medido num índice só, 1s sem nada rodando, 28s contra uma única transação aberta em OUTRA tabela, e 111s ao lado de um build concorrente vivo em outra tabela, terminando junto com ele. Quem lê isso como travado e interrompe piora o catálogo, e isso também é medição: o original fica `indisvalid = false` e aparece um `..._ccnew` ao lado, também inválido, então um índice morto vira dois. A mensagem tem que dizer que a espera é esperada e que não se interrompe, e não pode afirmar que um REINDEX que falha só pode ser violação de unicidade. **E um `..._ccnew` não é índice para reconstruir**: assim que a asserção admite que uma interrupção deixa um, a consulta de catálogo passa a listar o sobrante junto com o original, e "reindexe tudo o que a consulta devolver" transforma o sobrante num segundo índice válido. Medido: depois de interromper um REINDEX em `zz_r2`, reindexar os dois deixa `zz_r2` e `zz_r2_ccnew` válidos com `pg_get_indexdef` idêntico, duplicata permanente paga por toda escrita, e a guarda nunca mais reporta porque os dois estão válidos. A instrução tem que separar por NOME: qualquer coisa terminada em `..._ccnew` ou `..._ccold`, numerada ou não, se dropa, e o resto se reindexa. As três formas foram produzidas: interromper antes da troca deixa o original inválido mais `..._ccnew`; uma segunda interrupção, com o nome ocupado, acrescenta `..._ccnew1`; e interromper DEPOIS da troca deixa o índice real **válido** com um `..._ccold` inválido ao lado, que é a guarda disparando sem nada de errado com o índice. Esse último se alcança segurando `ACCESS SHARE` na tabela a partir de uma transação OCIOSA, que mantém o lock sem manter snapshot (`backend_xmin` nulo), então o REINDEX vence a espera inicial, troca, e para em `waiting for readers before marking dead`. Reconstruir qualquer um dos dois sufixos funciona e deixa duas cópias válidas idênticas, medido nos dois. **E o operador tem que ler o erro**: falta de disco, deadlock e timeout de statement ou de lock também derrubam um REINDEX, inclusive em índice não-único, e só `could not create unique index` quer dizer dado duplicado.

**E um build EM VOO lê exatamente como um cadáver.** O Postgres cria o índice inválido e valida depois, então um `CREATE INDEX CONCURRENTLY` rodando agora fica `indisvalid = false` pela duração inteira, e a asserção reprova o deploy em cima dele. Manter essa reprovação é o certo (adivinhar "alguém deve estar construindo" é como um cadáver de verdade passa), mas a mensagem tem que dizer como separar os dois, porque reindexar o build vivo de outra sessão é o movimento errado. O `indisready` **não** separa, e isso foi medido em vez de suposto: build morto durante a primeira varredura deixa `false/false`, o mesmo par que um build vivo mostra. O discriminador é o `pg_stat_progress_create_index`, que nomeia a tabela, o índice e a fase: `SELECT pid, relid::regclass, index_relid::regclass, phase FROM pg_stat_progress_create_index;`, sem `WHERE` (o motivo está duas frases abaixo, e o exemplo aqui já veio com o filtro uma vez). **Não** é casamento de texto de query no `pg_stat_activity`: um `CREATE UNIQUE INDEX CONCURRENTLY` vivo não casa `query ILIKE 'create index%'`, medido (a view de progresso nomeou o build e o ILIKE devolveu zero), então aquele conselho responde "nada rodando" justamente no caso que ele existe para pegar. **E a view responde diferente por papel: consulte-a SEM filtro.** Medido num banco cujo dono é papel não-superusuário, que é a forma de Postgres gerenciado do `docs/deploy.md`: contra um build iniciado por outro papel, o dono recebe a LINHA mas com todas as colunas nulas, enquanto o superusuário recebe tabela, índice e fase. Um `WHERE relid = '<tabela>'::regclass` descarta exatamente essa linha, e aí a checagem diz "nada rodando" para o operador para quem ela foi escrita. Sem filtro, a linha nulada ainda é resposta: alguma coisa está construindo, e você não tem privilégio para ver o quê.

**E os dois estados COEXISTEM, então a mensagem é uma SEQUÊNCIA e não uma bifurcação.** A tabela pode carregar um índice abandonado semana passada **e** ter um build sem relação rodando agora; a asserção lista os dois e a view de progresso nomeia só o segundo. Medido com um `zz_corpse` forjado ao lado de um `CREATE INDEX CONCURRENTLY "zz_live"` real preso em `waiting for old snapshots`: a asserção devolve dois nomes, a view devolve `conversations | zz_live` sozinho, e quando o build termina o `zz_live` fica válido e o `zz_corpse` continua `indisvalid = false`. Quem seguiu "tem build em voo, espere e redeploye" espera, resolve, redeploya e leva a mesma asserção na cara. **E particionar a lista por nome também não resolve**: "deixe em paz o que a view nomeia, reindexe todo o resto" quebra na linha nulada, que não nomeia nada, então para o dono sem `pg_read_all_stats` a regra manda reindexar justamente o build vivo que ele não consegue ver. A ordem que fecha os dois: esperar o que a view mostrar, **inclusive a linha nulada**, e só depois reindexar o que ainda estiver inválido, **derivando do catálogo** e não da lista da exceção nem da view (o catálogo é a única das três que nenhum arranjo de papel esconde do dono). E a espera precisa de teto, porque a view é cluster-wide: só interessa linha que PODERIA ser desta tabela, então linha nomeando outra tabela não conta, linha com `relid` saindo como OID cru é de outro banco do cluster e não conta, e só a linha toda nula é inatribuível. Contra uma enxurrada de linha nula existe saída medida: o mesmo papel, contra o mesmo build vivo, sai de `179174 | | |` para `179174 | conversations | zz_live2 | waiting for old snapshots` depois de um `GRANT pg_read_all_stats TO <papel>`, em sessão nova. Custa uma espera quando o build escondido era de outro banco do cluster, e é a única ordem em que nem o cadáver nem o build vivo é tratado errado.
...
**E a mensagem tem que fechar com o `resolve --rolled-back` FORA de qualquer ramo.** A linha da própria asserção fica FALHADA no instante em que ela lança, então o `migrate deploy` seguinte para com `P3009` independentemente do que o operador fez com o índice. Medido seguindo ao pé da letra o ramo "o build está em voo, espere e redeploye": o build termina e o re-deploy ainda sai 1. Runbook que acerta a parte difícil e perde um passo no fim deixa o operador exatamente onde o silêncio deixava. De quebra, um `migrate resolve --rolled-back` num nome que já tem linha rolled-back de um ciclo anterior **reporta sucesso e não mexe na linha aplicada**, o que produz corroboração falsa-negativa para quem estiver conferindo o ciclo.
