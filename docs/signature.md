...
**The consolidated retry asks its own question.** With `once` the question is whether the model already signed this *turn*, which only the original reply can answer, so a copy the customer received in an earlier balloon keeps the retry bare. With `all` the question is whether *this message* carries a copy, and handing it the original made the reply's opening copy answer for a remainder that merely starts with the same word.
