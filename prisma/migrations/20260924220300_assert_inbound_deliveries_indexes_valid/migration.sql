...
-- (tenant_id, id) CONCURRENTLY (issue #817). Same file as
-- `20260921120000_assert_conversation_indexes_valid`, pointed at `inbound_deliveries`; that file's
...
BEGIN
  SELECT string_agg(c.relname, ', ') INTO dead
   ...
   WHERE t.relname = 'inbound_deliveries' AND NOT i.indisvalid;
  IF dead IS NOT NULL THEN
    RAISE EXCEPTION
      'inbound_deliveries carries invalid index(es): %.
...
STEP 2, WAIT OUT WHAT COULD BE THIS TABLE''S: an index that query names on inbound_deliveries is being built right now, so leave it alone. A row naming another table, or one whose relid and index_relid render as bare OIDs instead of names (a build in another database of this cluster), is not yours and you do not wait for it. A row of all NULLs names nothing at all, so you cannot rule it out and you do wait. Re-run step 1 until nothing that could be this table''s is left. If nulled rows keep arriving, which on a busy cluster they will because step 1 is cluster-wide, have an admin run GRANT pg_read_all_stats TO <your role> and run step 1 again in a NEW session: the same role then reads the same build as inbound_deliveries | ..._idx | waiting for old snapshots instead of NULLs, and the rule above applies again.
STEP 3, REINDEX WHAT SURVIVES THE WAIT. Do not skip this because step 2 found a build: waiting repairs nothing, and an abandoned index sitting beside a live one raises this same assertion on the next deploy. Re-run this file''s own query, which is the one answer no role setup can hide from the owner (the list above is from before the wait, and step 1 can be nulls):
  SELECT c.relname FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid JOIN pg_class t ON t.oid = i.indrelid WHERE t.relname = ''inbound_deliveries'' AND NOT i.indisvalid;
...
STEP 4, ALWAYS: prisma migrate resolve --rolled-back 20260924220300_assert_inbound_deliveries_indexes_valid, then re-deploy. This migration''s own row is FAILED from the moment it raised, so without step 4 the next deploy stops with P3009 no matter what you did about the index.',
      dead;
  END IF;
END $$;
