...
- [`docs/api-and-fleet.md`](docs/api-and-fleet.md): the "one core, three transports" service pattern, read API v1, the **Bearer API keys in two scopes** (per-tenant, and fleet-scoped = a SUPER_ADMIN principal with no home tenant; a key answers the password step-up by itself), instance identity, and the **outbound** webhook substrate (`emitOutbound` + the single-replica delivery worker). Read when adding a read endpoint, an outbound event, a password-gated route, or touching the worker.
- [`docs/integrations.md`](docs/integrations.md): the integration catalog (TOOLPACK/MCP/NATIVE/WEBHOOK), the **generic inbound receptor** (`/api/v1/integrations/inbound/:routeToken`), pure mappers + registry, `agent_nudge`, and the **generic webhook** (`GENERIC`: an HTTP tool hands the operator's system `{{conversation_ref}}`, and the system speaks back into that conversation through the agent). Read when adding any integration or inbound webhook.
...
- [`docs/signature.md`](docs/signature.md): **the operator's signature** — the closing line the agent never writes, injected outside the model. Which four of the thirteen sends carry it and why the other nine do not, why it attaches to a CHUNK rather than to the text (both separators are what the splitter cuts on), and why the Chatwoot fork could not do it. Read before touching `src/modules/signature/*` or adding an outgoing send.
...
- [`docs/spend-ceiling.md`](docs/spend-ceiling.md): the **per-tenant spend ceiling** (`tenant.settings.spendCeiling`) — the calendar month's cost in USD as LANGFUSE costed it, read by a per-tenant `SPEND_CEILING_POLL` job into `spend_cost_snapshots` (the gate reads the row, never Langfuse: a Langfuse outage costs staleness, never a silenced tenant; the figure is monotonic inside the month; a block still written in tokens is no ceiling and says so), with SEPARATE ceilings for `inbox` and `playground` so testing cannot silence the agent for customers. Over it: the operator's sentence as the persona, a handoff, a private note; below it, a `warnAtPercent` warn on the FIRST verdict at or past the fraction. Which gate answers for which billed call is written down per ledger node in `spend-ceiling/coverage.ts` and fenced against `USAGE_NODE_IS_AGENT_TURN`. A refusal is only reported where a refusal HAPPENED: every gate sits after whatever would have stopped the call anyway (an unreadable file, a missing agent, a retired job, an empty burst) and immediately before the call. An unreadable ceiling ALLOWS the call. Read before touching `src/modules/spend-ceiling/*` or adding a billed call.

- [`docs/deploy.md`](docs/deploy.md): the deploy contract — the **two database roles** (superuser for migrate/bootstrap, non-superuser for runtime, with boot fail-fast), the boot ordering, and the **single-replica / one-leader** invariant. Read before touching Dockerfile/compose/`db-bootstrap`/boot sequence. **Three tests read this file and assert on its prose** (`tests/prisma/native-tool-names-renamed-by-migration.test.ts`, `delivery-route-observed-migration.test.ts`, `drop-retired-label-settings-migration.test.ts`), so editing it runs the full suite in CI while other docs do not, and a careless edit turns the suite red.
...
- [`docs/job-libraries.md`](docs/job-libraries.md): why the scheduler stays homegrown: what pg-boss and graphile-worker were measured doing on DEBOUNCE and FOLLOWUP (an expired job's retry runs beside the handler it could not stop; per-key serialization is released by expiry), what neither can express (the turn barrier's prefix claim, RLS, encrypted payloads), and what would reopen the question. Read before proposing a job library.
...
- Only add comments when strictly necessary, never obvious/redundant ones. **Where the tag goes**: a comment that DOCUMENTS a symbol (the module header at the top of a file, or the docstring directly above a declaration, exported or not) carries **no** tag; a comment INSIDE a body must have one: `// TODO:`, `// NOTE:`, or `// FIXME:`. Spelled out because the shorter phrasing reads as "every comment needs a tag", which makes review tooling flag every docstring in the tree. **Two positions are outside this**, and both are about the tag having nothing to mark. (1) **A JSX comment** documents the element directly below it, which is the docstring shape above rather than an aside inside a body, so it carries **no** `NOTE:` — `{/* Filters */}`, not `{/* NOTE: Filters */}`. `TODO:`/`FIXME:` stay available there, because work that is owed is owed in markup too. Enforced by `tests/lib/jsx-comment-tag-sweep.test.ts` rather than argued per PR (issue #553). (2) **A machine directive** is not prose and can never carry a tag: `biome-ignore` is only honoured at the start of its own comment, so `{/* NOTE: biome-ignore … */}` silently stops suppressing and the rule fires again (measured: a clean file goes to `Found 1 error`). The same holds for the `// t('key', 'Value')` comments the i18n extractor reads
