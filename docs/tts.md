...
**Its own model** (`buildSpeechNormalizer` in `graph/prepare.ts`). Four flat overrides on the block, `normalizeProvider` / `normalizeModel` / `normalizeCredentialRef` / `normalizeBaseURL`, each falling back to the agent's own model config INDEPENDENTLY (`resolveNormalizeModel`), so "same account, cheaper model" is a two-field change and an install that touches nothing keeps the exact behavior it had. The one non-field-wise fallback: once the provider actually CHANGES, an empty model resolves to the new provider's default rather than inheriting a model id that provider would refuse. Naming the SAME provider explicitly (what picking a separate credential on the same vendor looks like) keeps inheriting the agent's model, because swapping the key is not a request to swap the model. Temperature is pinned to 0 and the agent's `reasoningEffort` is dropped. A credential configured but unresolvable SKIPS the rewrite (visibly, `reason: credential_not_found`) instead of silently borrowing the agent's key on a provider that may not accept it, and a model config the factory rejects outright skips the same way (`reason: model_not_runnable`) rather than throwing: the runtime wraps the whole TTS branch in one try/catch, so an uncaught throw here would cost the customer the voice note, not just the rewrite.
