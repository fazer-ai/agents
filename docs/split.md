...
Absence, though, has to be *earned*, which is what the paging is for: `getMessages` answers with the newest ~20, so "absent from the newest twenty" is not absence. Three things end the walk with certainty — a send this same loop already completed (nothing older can carry an id minted after it), a page shorter than Chatwoot's (the top of the history, the same reading `debounce/handler.ts` makes of the same endpoint), or a rejection that never left the process (`ChatwootMissingTokenError`: there is no far side to ask). A ceiling of five pages and a single 10s budget bound the cost — the per-request deadline is what remains of that budget, so three pages is not three times the wait. Running out of either is `unknown`, never `absent`, and so is any page this build could not read whole.
