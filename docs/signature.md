...
**The flattened one keeps the line boundary**, and that was learned the hard way twice. It matches only whole **lines** of the balloon, and it runs only on a reply whose ends already say the model signed. Without the first, the signature `Alex\nSupport` collapses to `Alex Support` and the sentence "Alex Support can help with that." reads as already signed, so the balloon goes out with no closing at all: #599's round-12 hole coming back through the other door. Without the second, the same loose comparison is asked of every balloon of every reply, and two lines of ordinary text, a short list or an address, are one whitespace coincidence away from the same outcome. The price of both gates is a near-copy that differs from the configured signature in whitespace getting a second one, which is the trade this module makes every time: a duplicate over a missing closing.
