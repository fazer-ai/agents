...
- **A blank box is not always nothing.** The empty string is a VALUE for a string field (a PATCH that clears a provider field sends one), and a text input renders "left out" and "empty" identically. A REQUIRED string cannot be omitted, so blank there is the empty string and Send stays enabled; an OPTIONAL one reads as left out, with a one-line toggle under the blank box to say otherwise. `fieldTakesEmptyString` decides which types this applies to, and its test asks `parseToolInputSchema` the same question.
