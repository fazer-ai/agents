...
**A signature that spans a blank line is the exception**, and it is #599's split-boundary defect seen from the other side: the model's copy of it is cut by the same paragraph rule, so it occupies several balloons and no single one holds all of it. The whole-reply question still answers *whether* there is a copy, because it is the only one that can, and matching the signature's own paragraphs against the balloons at that end answers *which*. A run that does not match exactly claims nothing rather than guessing, so the worst case is the pre-existing one, a second signature, never a missing one. The original is the authority throughout: the split trims and throws the separators away, so balloons that merely reassemble into the signature are prose if the reply the model wrote was never signed.
