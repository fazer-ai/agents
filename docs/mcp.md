...
**What a tool description is for.** Two questions and nothing else: *when should a model pick this tool*, and *what must it get right for the call to be accepted*. Concretely, that is the tool's job in one line, the shape of its arguments, and the rules that cause a REFUSAL or change what the call means (a partial patch merging rather than replacing, `dry_run` defaulting to on, `credentialRef` taking a NAME or a `vault:<id>`, a field that must travel with another, a length cap that refuses rather than trims). What a feature *does*, why it exists, what it costs and what it writes to the trail belongs in `docs/` — the subsystem docs are where an operator looks anyway, and a paragraph here is paid for by every client on every session, before it knows whether the tool will be used at all.
