...
- **Dedicated fast worker** (`src/modules/debounce/worker.ts`, `DEBOUNCE_WORKER_INTERVAL_MS`, default 2500ms): drains **only** `DEBOUNCE` (`claimDueDebounceJobs`). The main scheduler tick (15s) claims everything **except** the two lanes that have their own worker (`claimDueJobs` → `kind NOT IN ('DEBOUNCE', 'MEMORY_COMPACT')`; the second is attendance compaction, split off for the opposite reason — see docs/graph.md), so the window is honored without ticking the reaper/sweep that fast. The scheduler's reaper still re-pends a stranded `CLAIMED` debounce job.
