...
**A transient vision failure is asked again, inside the budget one call used to have.** A 503 from the provider used to end the attachment permanently: `extract` was called once, and the "couldn't extract" marker is what entered the conversation history, so no later turn could recover the content (issue #319 — measured at 12 failures in 45 extractions on one tenant, nine of them 503, with the next turn succeeding 16s later on the same credential and model). `src/modules/vision/retry.ts` owns both halves of the answer: **which** failures are worth repeating (`408/429/500/502/503/504/529` and a timeout — never a 4xx about what we sent, and never a connection that would not open, which is as often a base URL that will never resolve), and **how long** each attempt may take. Up to three attempts with a jittered 500ms/1.5s backoff, all of it inside `VISION_TOTAL_BUDGET_MS` (60s, the value the single call already carried), so no turn can get slower than it already could. The per-attempt ceiling is per KIND: 20s for an image (measured live at 2.0-4.4s through gpt-4o, including a 3000x3000 one), and the whole budget for a document, which is up to 25MB and ~100 pages and has no live measurement here — a slow PDF keeps exactly the time it has today and still gains a retry whenever the failure leaves room. `stt` has the same gap and is NOT covered by this: no retry of a provider call exists anywhere else in the codebase.
