...
**The value is refused at the write**, like `enabled`, and since #618 so are `position` and `separator`. The tie-breaker is what GET does: the API echoes the settings bag as it was stored, so normalising in the reader would leave the operator's client reading `"sempre"` on a field the runtime answered as `"all"`. The three domains live in one leaf module, `src/modules/signature/domains.ts`, read by the runtime reader, the write boundary and the MCP patch schema; before that each spelled its own copy and the boundary spelled one of three. A test asks the reader, by execution, that every allowed value round-trips and a value outside is replaced, so a domain that grows on one side only fails there. The refusal is per field and only for a value the write introduces or changes, compared by value, so a legacy row re-sent untouched still saves and the reader's normalisation stays for it.
...
With `all` the dedupe becomes a **per-balloon** question, and it has to: `alreadySigned` asks about the reply as it arose, at both ends, which is right for `once` and wrong here, because a model that signed itself at the end would suppress the badge on every other balloon. The rule inside a balloon is unchanged, line boundary included.

**A signature the splitter took apart is the exception**, and it arrives in more than one shape: one containing a blank line is cut by the paragraph rule, one longer than `maxChars` is cut by the **sentence** rule with no blank line involved, and at the `maxChunks` ceiling the overflow is merged back with each paragraph trimmed and the original separator run kept. Review rounds 1, 3 and 4 each reported one of those, which is the signal that chasing them one at a time is the wrong shape: every one of them differs from what the model wrote only in **whitespace**.

So there is one question, asked through one normalisation. `alreadySigned`'s own two comparisons say which **end** of the reply carries a copy, exactly, on the reply as it arose; a reply the model never signed answers no to both, and that is what keeps all of this off prose. From each end that says yes, a walk goes inward over balloons that are still a suffix (or a prefix) of the signature once whitespace is collapsed on both sides, and stops at the first balloon that is not. That stop is how a balloon holding content **and** a fragment keeps its own signature: suppressing the signature on a balloon the customer reads as content is the failure this feature exists to prevent, so the worst case stays a second signature, never a missing one. **Both** ends are walked, because a model that opened and closed with the same line wrote two copies.

Two per-balloon checks finish it. The exact one covers a copy the walks cannot reach, in the **middle** of the reply, where no walk starts. The flattened one covers a whole copy that shares a balloon with prose, which the `maxChunks` merge produces by trimming the paragraphs and keeping the newlines between them.

**The normalisation for a single balloon keeps the line breaks**, and that was learned twice over. It drops indentation and blank lines, which is exactly what the merge introduces, and keeps every newline, which is what the merge preserves. It then matches the signature's own lines against a run at one **end** of the balloon, never in the middle, and only on a reply whose ends already say the model signed.

Each of those three is a failure someone reported. Collapse the breaks and the two-line signature `Alex\nSupport` becomes `Alex Support`, so the sentence "Alex Support can help with that." reads as already signed and the balloon goes out with no closing at all, which is #599's round-12 hole coming back through the other door. Match in the middle instead of at an end and any balloon that mentions the signature's words loses its own. Drop the gate and the comparison is asked of every balloon of every reply, where two lines of ordinary text, a short list or an address, are one coincidence away from the same outcome.

The trade behind every one of those decisions is the same, and it is the module's standing one: **a duplicate over a missing closing.** A second signature is visible and ugly; a balloon with none is the failure the feature exists to prevent.

**The consolidated retry carries a signature if and only if the balloons it replaces did.** Asking the balloons themselves is the only rule that cannot disagree with the decision the pass already made; handing the retry the whole original reply instead let the reply's opening copy answer for a remainder that merely starts with the same word, and the retry went out bare.

Whether the retry **already** carries a copy is a second question, and it is `attachSignature`'s own, asked of the retry's own text through the same normalisation. Reading it off the balloons does not work: the walk leaves every balloon of a multi-balloon copy alone, so an untouched balloon may hold a **fragment** rather than the whole closing, and a retry carrying only the fragment went out with no name on it.

**The consolidated retry is one message, so it carries one signature.** When a send fails and the remainder is retried as a single message, it is built from the *raw* balloons and signed once, not from balloons that already carry one each, which put the badge three times inside one body. And which message carries it does not change because a send failed: `once` with `top` whose first balloon already landed does not come back on the retry. The balloon **count** is the invariant repetition must not touch: `deliverReply` keeps `seps` aligned with `chunks` by index, so a signature that added or merged a balloon would misalign every pause after it.
