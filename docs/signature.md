...
The price of all three is a near-copy that differs from the configured signature in whitespace getting a second one, which is the trade this module makes every time: a duplicate over a missing closing.

**The consolidated retry carries a signature if and only if the balloons it replaces did.** That is the only rule that cannot disagree with the decision the balloon pass already made, and two others were tried first: handing the retry the whole original reply let the opening copy answer for a remainder that merely starts with the same word, and handing it its own lossy reconstruction disabled the dedupe for a copy the balloon pass had correctly recognised, so a second signature went out. It also says the `once` rule without naming it: a `top` signature whose first balloon already landed is not among the balloons being retried, so none of them was signed and neither is the retry.

With one more clause, because **one message means one signature**: a retry spanning several balloons can already contain the model's own copy inside one of them, and "some balloon was signed" is then true and not enough. With `all` every non-blank balloon is answered, signed or left alone because it *is* a copy, so a balloon the pass did not touch is the evidence, and it is the only evidence that survives the lossy rejoin. `once` touches exactly one balloon by design, so the same reading there would call every other balloon a copy and silence a retry that should carry the closing.
