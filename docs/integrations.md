...
- **Availability spans SEVERAL calendars** when the instance allows several and the model omits `calendarId` (issue #100): one `freeBusy` request carries every allowed calendar, each is computed on its OWN busy list, and the merged list is chronological with each slot tagged `calendarId`/`calendarLabel`. Calendars are independent providers here, so their busy windows are never pooled: pooling would answer when they are ALL free at once, which is a meeting room, not a clinic. `freebusy.query` caps `calendarExpansionMax` at 50, so the queried set is trimmed to the first 50 allowed calendars and the tail is reported as unavailable rather than letting Google answer `tooManyCalendarsRequested` for an arbitrary subset. A calendar `freeBusy` reports an error for is dropped and named in `unavailableCalendars` rather than carried as free (an empty busy list means "free all day", and offering a professional whose bookings we cannot see is a double booking); if none could be read the tool refuses, because an empty slot list reads as "fully booked". Booking, rescheduling and cancelling still act on ONE explicit calendar. Nothing bounds the merged list: an earlier revision kept each calendar's first few starts and that quietly turned "all bookable slots" into "the first couple of hours" (eight starts is under two hours at the default 15-minute grain), so an afternoon request answered unavailable while the afternoon was free. A blocking calendar that is ALSO an operable one is read and applied to its siblings but not to itself: `freeBusy` ignores the transparent/all-day events a closure is made of, so skipping the read would make that closure invisible to everyone.
