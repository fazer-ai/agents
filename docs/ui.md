...
- **The preview follows the runtime's gate, not a friendlier one.** `templatePreviewFor` takes the STATUS the sample came back under (null for a hand-pasted one, which reads as 2xx) because `projectResponse` runs on 2xx alone: a sample captured from a 404 the tool declares a result is previewed raw and clipped, under a line saying why. Otherwise the box labelled "exactly what the agent would receive" contradicts the `modelText` the same test run reported one dialog away. The clip length is `MODEL_RESPONSE_CHAR_LIMIT`, imported from the same module `graph/tools/http.ts` clips with — a second 4000 here would be a second answer to one question, and the wrong one would be the one nobody is looking at.
