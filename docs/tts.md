...
**Where the spoken words are kept, and why `content` is not one of the places** (issue #763). An audio reply leaves the Chatwoot message `content` EMPTY, and every reader that only looks there concludes the agent said nothing: that is how a batch of ten conversations showed as "customer got no answer" while the agent had answered five times by voice. Filling `content` is the obvious fix and it is the one that breaks delivery. The fork hands the WhatsApp connector `caption: message.outgoing_content` for every media kind (`Whatsapp::Session::Outbound::AttachmentAdapter`), and the connector refuses a caption on an audio, because `AudioMessage` has no such field in the protocol: `no caption travels with audio: send the text as its own message`, `ErrorInvalidPayload`, proved by its own suite for a plain audio and for a voice note. So a filled `content` does not leak a caption to the customer, it fails the send and the customer receives nothing.

The words are therefore kept in the two places a later reader can reach without a caption:

- **On the attachment**, as `attachments_metadata[{file}][transcribed_text]` (above). The fork stores it in the attachment's `meta` and the dashboard renders it under the player, which is what the operator reading the conversation needs. **Upstream Chatwoot has no meta route and drops it silently** — the same gap that makes the inbound STT pass keep an in-process overlay (issue #49).
- **On the `tts` flow-log line**, as `detail.text`: the reply as the agent wrote it, which is the object every length/markdown/required-content/safety check is about. When the speech rewrite changed it, `detail.spoken` carries what the provider was actually handed, because `normalized: true` says a rewrite RAN and not what it produced. This is the one reader that answers on either Chatwoot, which is why an assessment should read it rather than the message `content`.

The runtime also stashes the same words in the media-annotation overlay against the id of the message it just sent, so a debounce flush or a delivery recovery re-reading the page sees its own audio reply as words instead of an empty outgoing row, on upstream too. The overlay is in-process with a 15-minute TTL: it covers the turns that follow immediately, never a later audit.
