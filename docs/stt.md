...
**EVERY attachment is analyzed, not the first (issue #691).** The eager pass took `firstVisualAttachment`, which did what its name said: a customer who attaches the receipt, the ID and a screenshot in one message had one of the three described, and the reply asked for what was in the other two. Nothing reported it — the `vision` stage wrote one `ok` line per turn, because from the pipeline's point of view nothing had been skipped. Measured on one production mailbox over 30 days, **50.6% of the conversations that arrive with an attachment carry more than one** (3,667 conversations, mean 1.98, max 70). The extractions run in PARALLEL, because the per-file budget is 20s for an image and 60s for a document and five of those in series is a turn nobody waits for; a cap of **8 per message** bounds the album case, and the overflow is NAMED to the model (`[N anexo(s) além do limite … não foram lidos]`) rather than dropped, since a model told nothing answers as if the message had those files fewer. `parseChatwootMessages` reads the meta of every attachment and labels each part with its file name when there is more than one, so two receipts read as two orders instead of one document contradicting itself; a single attachment keeps the bare text, byte for byte. One unreadable file never costs the others. `firstAudioAttachment` still has this shape and is NOT covered: several audio attachments in one message transcribe one.
