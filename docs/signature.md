...
Two per-balloon checks finish it. The exact one covers a copy the walks cannot reach, in the **middle** of the reply. The flattened one covers a whole copy that shares a balloon with prose, which the `maxChunks` merge produces, and it is restricted to a signature spanning **more than one line**: collapsing whitespace turns the line boundary into a space, and for a one-line signature that is #599's round-12 hole coming back through the other door, where `chame o Alex` would read as already signed and the balloon would go out bare. Both are boundary checks and never containment, for the same reason.
