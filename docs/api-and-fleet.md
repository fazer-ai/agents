...
| minted by | a TENANT_ADMIN session, confirmed with the current password (a tenant key mints one by itself: it inherits the step-up its own minting answered) | a SUPER_ADMIN **session**, confirmed with the current password. A key is refused (`errors.fleetApiKeyRequiresSession`): a leaked key must not be able to outlive its own revocation by minting another |
...
**Step-up is a property of the session, and a Bearer key answers it by itself.** The six password-gated routes (`POST /v1/api-keys`, `DELETE /v1/tenants/:id`, `DELETE /v1/agents/:id`, `DELETE /v1/chatwoot/deployment`, `POST /v1/chatwoot/instances/:id/remove`, `DELETE /admin/users/:id`) go through `confirmStepUp` (`src/api/lib/step-up.ts`): a session sends `password` (omitted → 400 `errors.passwordRequired`, wrong → 403 `errors.invalidPassword`), an API-key principal omits it and is not asked. They used to verify the password against the row `ctx.userId` names, which for a key is its CREATOR, so an automation could only pass by holding a person's password as well — the coupling #308 reports as fragile, and one that proved nothing the key had not already proved: it is presented on every request, and it was minted under a step-up. That last half is what makes minting a key one of the six: a seven-day session that could mint a key without a password would carry the key past the rule on every destructive route (review round 1 on #308). The name confirmation (`confirmName`/`confirmDomain`) stays for every principal: it is about the target, not the actor. `tests/api/lib/step-up.test.ts` holds every `verifyPassword(` call in `src/api` to the login route and the helper.
