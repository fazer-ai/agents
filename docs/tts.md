...
- **A model id or a credential with the provider left inherited**: refused (`override_without_provider`). Nothing records which vendor either was chosen for, so the next change to the agent's provider re-points a key at one that never issued it and asks a model id of one that has never heard of it — and that change does not need REST: the General tab saves on its own, so the Behavior tab's cleared state never reaches the database, and a REST `PATCH` of `modelConfig` does not touch the settings bag at all. Naming the provider, even the agent's own, pins them in the settings bag; the editor does it for the operator the moment either field is set. Only a rewrite that overrides nothing may leave it blank, and that one has nothing to go stale.
...
Which key travels is therefore an explicit three-way choice: `own` (a credential configured for the rewrite), `agent` (only to the agent's own destination), and `none` — no key at all, reachable for `openai-compatible`, which authenticates through its base URL. That third state is what lets a local llama.cpp-style rewrite run without inventing a dummy vault entry, and without the agent's key being sent to it. A configuration with none of the three available is refused (`credential_required`), as is an `openai-compatible` with no endpoint of its own to reach (`endpoint_missing`, which `createChatModel` would otherwise throw on) and an endpoint configured for a provider whose adapter DROPS it (`endpoint_unsupported`: only `openai-compatible` and `openrouter` send one, and the rest take the value and leave for the vendor's public host carrying the key and the customer's text — `PROVIDERS_HONORING_BASE_URL`, probed against the built instances so the list cannot drift). An endpoint INHERITED from the agent is never judged that way: honored or dropped, the rewrite lands wherever the agent's own model lands; an unset model on a switched provider resolves to that provider's own default (the #94 lesson: an empty name travels verbatim and the call is refused).

The editor warns before any of that can be saved, but REST and MCP write the settings bag directly, so the refusal lives in the resolver, not in the UI — and `computeConfigIssues` raises a config-health issue for EVERY refusal, not only the ones about a key, since a typo'd provider name and a missing endpoint kill the rewrite just as silently and the editor cannot produce either. One editor rule exists for the same reason and is worth knowing: switching the REWRITE's provider clears the model, key and endpoint picked for the old one, including the base-URL state kept OUTSIDE the form, which was restoring the previous endpoint behind a field that no longer renders. Switching the AGENT's provider deliberately clears NOTHING: an override that named its vendor is still pinned to it, one that named nothing has nothing to go stale, and reaching across tabs to clear a field would destroy it un-undoably (the General tab's Discard restores only the General tab). Temperature is pinned to 0 and the agent's `reasoningEffort` is dropped. Every other way the build can fail also skips visibly rather than throwing (`credential_not_found`, `model_not_runnable`): the runtime wraps the whole TTS branch in one try/catch, so an uncaught throw would cost the customer the voice note, not just the rewrite.
