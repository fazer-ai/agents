...
> = {
  ...
  // The balloon's own LINES, cut from the array `split("\n")` returned, to ask whether a whole run
  // of them is the model's copy of the signature. An array of strings, never a string, so no cut
  // can land inside a code point; and the pieces are compared, never sent.
  "src/modules/signature/service.ts": [1, "array"],
