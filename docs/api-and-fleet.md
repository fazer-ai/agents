...
**A projection carries the SHAPE of a free-form field, never its contents.** Half the audited families own at least one column the writer does not allowlist — an integration's `config` (`z.record(z.string(), z.unknown())`, two of whose keys are read back as HTTP header names), an experiment's variant `systemPrompt`, a document template's blocks. A row is append-only and readable by every tenant admin, so it outlives every correction to what it copied. The rule those three settle on is one rule: project the structure and not the value. `config` contributes its sorted KEYS; a variant contributes `{key, weight}`; a template contributes `{id, type}` per block plus a `contentDigest` over blocks+fields+style. The digest is what keeps an edit INSIDE a block visible at all — the structure does not move when text is replaced, so without it `projectionMoved` would see nothing and the commonest edit a template gets would write no row. #399.
