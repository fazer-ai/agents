...
2. **Monotonic watermark CAS**: advance `lastHandledMessageId` to `targetWatermark` only if the stored value is `null`/`< target`. The loser of a (rare) concurrent claim sees 0 rows and suppresses, so a burst is answered **at most once**. The watermark advances on the success path (post), so an LLM/Chatwoot error retries against the same burst (idempotent).
   ...
   The same winning CAS also advances **`lastAnsweredMessageId`**, but only from the two post gates (`shouldPost` here and its twin on the direct path). The two columns answer different questions and the second exists because the first cannot: `lastHandledMessageId` means "never re-ANSWER this" and most of its writers advance it precisely because **no turn is running** (an unauthorized contact, a human taking the conversation, a burst that renders to nothing), so a reader asking "did anything look at this message" gets a yes when nothing did. `lastAnsweredMessageId` says a turn **ran** covering up to that message — not that a reply was delivered, since the claim precedes the send. The stranded-delivery sweep is the reader that needs the distinction (`docs/chatwoot.md`, issue #228).
