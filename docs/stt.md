...
**A transient vision failure is asked again, inside the budget one call used to have.** A 503 from the provider used to end the attachment permanently: `extract` was called once, and the "couldn't extract" marker is what entered the conversation history, so no later turn could recover the content (issue #319 — measured at 12 failures in 45 extractions on one tenant, nine of them 503, with the next turn succeeding 16s later on the same credential and model). `src/modules/vision/retry.ts` owns both halves of the answer: **which** failures are worth repeating (`408/429/500/502/503/504/529` and a timeout — never a 4xx about what we sent, and never a connection that would not open, which is as often a base URL that will never resolve), and **how long** each attempt may take. Two attempts with a jittered 500ms backoff, all of it inside `VISION_TOTAL_BUDGET_MS` (60s, the value the single call already carried), so no turn can get slower than it already could. The **non-final** attempt is capped per KIND — 20s for an image (measured live at 2.0-4.4s through gpt-4o, including a 3000x3000 one), the whole budget for a document (up to 25MB and ~100 pages, with no live measurement here). The **last** attempt is capped only by what is left of the total, and that is deliberate: the per-kind ceiling exists to leave room for a next attempt, and after the last one there is none — capping it there would buy nothing and would cost the one case a short ceiling is bad at, a call that is legitimately slow (a loaded vendor, a self-hosted `openai-compatible` endpoint on modest hardware) which used to have the whole 60s. That is also why there are two attempts and not three: a third would have to come out of the same total, and what it costs is the length of the last one. `stt` has the same gap and is NOT covered by this: no retry of a provider call exists anywhere else in the codebase.
