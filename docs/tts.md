...
- **Same destination** (same vendor AND same host): each unset field falls back to the agent's, field by field, and the call runs on the agent's key. "Same account, cheaper model" is a one-field change, and an install that touches nothing behaves exactly as before. Swapping the key is not a request to swap the model, so the agent's model still applies.
...
- **Different host, same vendor**: the agent's key was not issued for that host either, so it is the same leak with a smaller radius. Pointing the rewrite at a proxy on purpose is still supported — name the credential, even the agent's own, and the intent is explicit rather than inferred.
