...
> = {
  ...
  // The audit fingerprint of a template's content: a hex digest, so the cut cannot land inside a
  // surrogate pair. Same shape and same reason as tenant-settings' over-ceiling sentence (#399).
  "src/modules/documents/templates.ts": [1, "ascii"],
