...
**A family moves down as a whole, and what is a READ falls out of it on the way.** #395 moved the channel trail into `src/modules/chatwoot/management.ts`, and three of its eleven console routes had no MCP twin at all, so `deployment.disconnect`, `instance.reconnect` and `instance.remove` are named there for the first time: before it, disconnecting a whole deployment (which cascades every account, inbox, bot and conversation of the tenant, and deletes its contacts by hand first) left nothing anywhere. The row for that one counts what it is about to destroy, inside the same transaction and before the delete. Moving the family also settled what does NOT move: `inbox.reconcile` was audited by the MCP transport and `reconcileInboxBots` writes nothing on either side (its REST twin is a `GET`), so the row recorded a read, and pushing it down would have spread it to two more doors. It joins `webhook_test` and `mcp_connection_discover` in recording nothing. The deployment admin token stays out of every projection on both paths, which a fence over the whole family's rows measures rather than assumes. #395.
