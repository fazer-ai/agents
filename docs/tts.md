...
**Its own model** (`buildSpeechNormalizer` in `graph/prepare.ts`). Four flat overrides on the block, `normalizeProvider` / `normalizeModel` / `normalizeCredentialRef` / `normalizeBaseURL`, each falling back to the agent's own model config INDEPENDENTLY (`resolveNormalizeModel`), so "same account, cheaper model" is a two-field change and an install that touches nothing keeps the exact behavior it had. The one non-field-wise fallback: once a DIFFERENT provider is picked, an empty model resolves to THAT provider's default rather than inheriting a model id the new provider would refuse. Temperature is pinned to 0 and the agent's `reasoningEffort` is dropped. A credential configured but unresolvable SKIPS the rewrite (visibly, `reason: credential_not_found`) instead of silently borrowing the agent's key on a provider that may not accept it.
