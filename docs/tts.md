...
Five optional per-agent knobs control HOW the words come out, as opposed to which words the model picked: `stability` (0-1; low = expressive, high = monotone — the usual cause of a "robotic" voice note), `similarityBoost` (0-1), `style` (0-1; costs latency and destabilizes high), `speed` (0.7-1.2, the band ElevenLabs accepts) and `speakerBoost`. They live **flat** on `agent.settings.tts`, not in a nested object, because `mergeBehaviorSettings` merges a block shallowly — nesting them would make a patch of one knob null out the rest and break the partial-patch contract the REST/MCP transports promise. `voiceSettingsOf` regroups them at the provider boundary and returns **null when the operator set none**, so the adapter omits `voice_settings` entirely and the request body of an untouched agent stays byte-identical to what it was before this existed (ElevenLabs then falls back to the settings saved on the voice itself). `readTtsConfig` **clamps** out-of-range values rather than rejecting the write — an overshot slider is not a reason to fail a settings save. Only ElevenLabs consumes them; OpenAI's `/audio/speech` has no equivalent bag and the mapper ignores what it cannot express.

## Writing for the ear (`buildSpeechContextSection`)

`prepareSpeechText` strips the markup and `llmNormalizeForSpeech` fixes the numbers, but **neither restructures a sentence** — a clipped, list-shaped reply ("tenho 08:00, 08:30 e 09:00") still sounds like a machine reading a form no matter how good the voice is. `userSentAudio` is known BEFORE generation on both real paths, yet used to be read only after, to pick text vs audio; the model therefore wrote for the eye and the reply was spoken anyway. `buildModelAndGraph` now takes a `GraphTurnContext` and appends the write-for-the-ear section when the turn will be spoken. It is **not** threaded through `LoadAgentArgs` on purpose: the debounce flush loads the config in `flushDebounceJob`, before `coalesceAndRunTurn` selects the burst, so the flag does not exist yet at load time. The playground deliberately does not pass it — it never synthesizes, so claiming "this reply will be spoken" there would be a lie.
