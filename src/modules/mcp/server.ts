...
export function buildMcpServer(principal: VerifiedToken): McpServer {
  ...
  if (hasScope(principal, "mcp:write")) {
    ...
    registerTenantTool(
      ...
      {
        description:
          "Update an agent's name/enabled/mode/transfer_with_summary/model_config/business_hours_id/follow_up_hours_id. mode is 'test' (silent until /teste) or 'production' (answers). Previews a diff and applies NOTHING unless dry_run is false. (System prompt → prompt_set; behavior → agent_settings_set.) model_config credentialRef accepts a vault NAME.",
        inputSchema: {
          agent_id: z.string(),
          name: z.string().optional(),
          enabled: z.boolean().optional(),
